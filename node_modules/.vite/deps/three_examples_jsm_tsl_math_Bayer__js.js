import {
  Fn,
  float,
  floor,
  int,
  ivec2,
  mod,
  screenSize,
  screenUV,
  textureLoad,
  vec3
} from "./chunk-PDWB67LD.js";
import "./chunk-25TV67T5.js";
import "./chunk-ID4BRPOZ.js";
import {
  TextureLoader
} from "./chunk-H2TYGQAY.js";

// node_modules/three/examples/jsm/tsl/math/Bayer.js
var bayer16Texture = null;
var bayer16 = Fn(([uv]) => {
  if (bayer16Texture === null) {
    const bayer16Base64 = "data:image/png;base64,";
    bayer16Texture = new TextureLoader().load(bayer16Base64);
  }
  return textureLoad(bayer16Texture, ivec2(uv).mod(int(16)));
});
var bayerDither = Fn(([color, steps = float(32)]) => {
  const screenPos = screenUV.mul(screenSize);
  const x = mod(floor(screenPos.x), float(4));
  const y = mod(floor(screenPos.y), float(4));
  const bayer = mod(
    floor(x.add(1)).mul(floor(y.add(1))).mul(17),
    16
  ).div(16).sub(0.5);
  const ditherOffset = bayer.div(steps);
  return vec3(
    color.r.add(ditherOffset),
    color.g.add(ditherOffset),
    color.b.add(ditherOffset)
  );
});
export {
  bayer16,
  bayerDither
};
//# sourceMappingURL=three_examples_jsm_tsl_math_Bayer__js.js.map
